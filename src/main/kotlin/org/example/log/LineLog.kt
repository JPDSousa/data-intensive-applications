...
val lineLogQ = named(LineLogFactory.LineLog::class.qualifiedName!!)
...
class LineLogFactory(private val pathTrash: Trash<Path>): LogFactory<String, LineLogFactory.LineLog> {
    ...
    inner class LineLog(
        ...
        override fun <T> useEntriesWithOffset(
            ...
            false -> path.readOnly { file ->
                file.apply { seek(offset) }
    ...
    private inner class LineLogTrash: Trash<LineLog> {

        override fun mark(deleteMe: LineLog) = pathTrash.mark(deleteMe.path)
    }

    override val trash: Trash<LineLog> = LineLogTrash()

    override fun create(logPath: Path): LineLog {
